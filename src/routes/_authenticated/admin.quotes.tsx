  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <p className="mt-6 text-sm text-muted-foreground">No quote requests yet.</p>
      ) : (
            ...
            <div key={r.id} className="rounded-xl border border-border bg-card p-4 shadow-soft">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  {r.phone && <p className="mt-1 text-sm">Phone: {r.phone}</p>}
                  {r.service && <p className="text-sm">Service: {r.service}</p>}
                  {(r.quantity || r.turnaround) && (
                    <p className="text-sm">{r.quantity} · {r.turnaround}</p>
                  )}
